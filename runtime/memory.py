#!/usr/bin/env python3
"""
AGENT MEMORY — Three-Tier Memory System
...
class EmbeddingProvider:
    ...
    def _local_embed(self, text: str) -> Optional[List[float]]:
        ...
        Returns None when sentence-transformers isn't installed or model
        loading fails (no hard dependency — local embedding is optional).
        Install support with:  pip install sentence-transformers
        """
        try:
        ...
        # Cache the model on the instance so repeated calls don't reload it.
...
if __name__ == '__main__':
    print("the agent Memory System")
    print("=" * 40)
    
    manager = get_memory_manager()
    
    # Test
    manager.remember("Met with the operator today", context="first meeting", importance=8)
    manager.remember("the operator likes AI", memory_type='semantic', importance=7)
