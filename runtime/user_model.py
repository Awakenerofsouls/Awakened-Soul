#!/usr/bin/env python3
"""
user_model.py — Model of the user.
...
class UserModel:
    ...
    def __init__(self, path: Path = None):
        # Default path is `user_behavior.json`, NOT `user_model.json` —
        # the latter is owned by psychological_state.UserModel which uses
        # a different schema. Using distinct files prevents schema collisions
        # when both classes are instantiated in the same process.
        self.path = path or Path(os.getenv("AGENT_HOME", str(Path.home() / ".agent"))) / "user_behavior.json"
    ...
    def update_from_message(self, user_message: str, is_correction: bool = False):
        ...
        # Recompute correction_rate so the "corrector" trait actually fires.
        total = b.get("total_messages", 0) or 1
        b["correction_rate"] = b.get("corrections", 0) / total

        # Update curiosity (questions)
        if "?" in user_message:
            b["curiosity_score"] = min(1.0, b["curiosity_score"] + 0.05)

        # Update detail level
        word_count = len(user_message.split())
        if word_count > 50:
            b["detail_level"] = min(1.0, b["detail_level"] + 0.02)
