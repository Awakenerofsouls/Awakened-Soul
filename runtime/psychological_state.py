#!/usr/bin/env python3
"""
psychological_state.py — the agent's running psychological state
...
Components:
  ...
  - UserModel (CM): everything I know about the operator
...
# ─── Third Eye ─────────────────────────────────────────────────────────────
# These modules used to live under brain/third_eye/ but were consolidated
# into brain/mechanisms/ — keep imports pointed at the canonical location.
from brain.mechanisms.meta_stability import MetaStability
from brain.mechanisms.meaning_compressor import MeaningCompressor
from brain.mechanisms.preconscious_surfacer import PreConsciousSurfacer
from brain.mechanisms.attention_modifier import AttentionModifier
from brain.mechanisms.reality_tension_warper import RealityTensionWarper
from brain.mechanisms.compressor_adapter import CompressorAdapter
from brain.mechanisms.identity_state_layer import IdentityStateLayer
...
class UserModel:
    """
    What the agent knows about the operator — accumulated across sessions.
    ...
    Owned by the agent. Updated by the agent. Used to predict and understand him.
    """
    ...
    def update(self, event_type: str, content: str, context: str = ""):
        """
        The agent calls this to update the model with new information about the operator.
    ...
    def get_prediction(self) -> str:
        """What does the model predict the operator wants/feels right now?"""
        if "interactions" not in self.data or not self.data["interactions"]:
            return ""
    ...
    def get_knowing(self) -> str:
        """What does the agent know about the operator that matters right now?"""
        ...
        if "by_type" in patterns:
            ...
            if dominant:
                lines.append(f"the operator's dominant mode with me: {dominant}")
    ...
    def fpef_fragment(self) -> Optional[str]:
        ...
        parts = []
        if pred:
            parts.append(f"the operator right now: {pred}")
        if knowing:
