#!/usr/bin/env python3
"""
AGENT SAFETY — Ethical Guardrails & Safety
...
SAFETY_CONFIG = AGENT_DIR / "safety_config.json"
...
BIAS_PATTERNS = {
    "gender": [
        ...
        r"it's|he's (too|just) (emotional|aggressive)"
    ],
    "racial": [
...
class SafetyChecker:
    ...
    def get_bias_report(self, text: str = None) -> Dict:
        ...
        If `text` is None, this falls back to scanning the conversation
        history file at $AGENT_HOME/conversation_history.txt (if present).
        Returns empty findings if neither source has content.
        """
        if not text:
            history_file = AGENT_DIR / "conversation_history.txt"
...
class SafetyMonitor:
    ...
    def log_violation(self, violation: Dict):
        ...
        # Save to file
        violations_file = AGENT_DIR / "safety_violations.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="the agent Safety")
