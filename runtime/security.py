#!/usr/bin/env python3
"""
security.py — Security layer for the agent.
...
class SecurityLayer:
    ...
    def check_loop_risk(self, action_count: int, time_window: int = 60,
                        rate_threshold_per_minute: int = 100) -> bool:
        """Check for runaway cognitive loop risk.

        Risky if `action_count` actions in `time_window` seconds exceeds
        `rate_threshold_per_minute` actions/minute. Default threshold = 100/min.
        """
        if time_window <= 0:
            return action_count > rate_threshold_per_minute
        actions_per_minute = action_count * 60.0 / time_window
        return actions_per_minute > rate_threshold_per_minute
