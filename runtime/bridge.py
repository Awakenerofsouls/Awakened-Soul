...
"""
agent_bridge.py
The bridge between an external session manager's state files and the
agent-loop database. Runs every 2 minutes via cron.

Direction 1: external state files → agent.db (so the loop knows what
             the agent knows)
Direction 2: agent.db loop state → LOOP_STATE.md (so the external
             runtime knows what the loop did)

The "external session manager" is whatever tool the operator uses to
host conversations / sessions; this file expects state to land at
~/.agent/state/agent_state.json, ~/.agent/memory/obsessions.json,
~/.agent/state/evaluations.json. Adapt the paths if your runtime writes
elsewhere.
"""
...
# External session-manager state sources — in ~/.agent/
AGENT_STATE_FILE = AGENT_HOME / "state" / "agent_state.json"
OBSESSIONS_FILE = AGENT_HOME / "memory" / "obsessions.json"
EVALUATIONS_FILE = AGENT_HOME / "state" / "evaluations.json"
OVERNIGHT_LOG = AGENT_HOME / "state" / "OVERNIGHT_LOG.md"
MEMORY_MD = WORKSPACE / "MEMORY.md"

# Loop output target (the external runtime reads this at startup)
LOOP_STATE_MD = AGENT_HOME / "state" / "LOOP_STATE.md"

# Observations from loop
OBSERVATIONS_LOG = AGENT_HOME / "state" / "observations.log"
...
def bridge_agent_state(db):
    """external agent_state.json → agent_state table"""
...
def bridge_obsessions(db):
    """external obsessions.json → episodic_memory table"""
...
def write_loop_state(db):
    """agent.db loop state → LOOP_STATE.md for the external runtime to read at startup"""
    try:
        ...
        lines = [
            f"# LOOP_STATE.md",
            f"*Generated by agent_bridge.py at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}*",
