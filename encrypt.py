#!/usr/bin/env python3
"""
AGENT ENCRYPT — Vault Encryption for Identity and Memory
...
# Configuration
AGENT_DIR = Path(os.getenv("AGENT_HOME", os.getenv("AGENT_HOME", str(Path.home() / ".agent"))))
IDENTITY_FILE = AGENT_DIR / "IDENTITY.md"
MEMORY_DIR = AGENT_DIR / "memory"
ENCRYPTED_DIR = AGENT_DIR / "encrypted"
VAULT_CONFIG = AGENT_DIR / "vault.json"
...
def encrypt_file(file_path: Path, key: bytes) -> Path:
    ...
    relative = file_path.relative_to(AGENT_DIR).as_posix().replace("/", "__")
...
def encrypt_vault(passphrase: str = None):
    ...
    if IDENTITY_FILE.exists():
        ...
        if encrypted_path:
            files_encrypted.append(str(IDENTITY_FILE.relative_to(AGENT_DIR)))
            # Remove original
    ...
    if MEMORY_DIR.exists():
        for mem_file in MEMORY_DIR.glob("*.md"):
            ...
            if encrypted_path:
                files_encrypted.append(str(mem_file.relative_to(AGENT_DIR)))
                # Remove original
                mem_file.unlink()
    ...
    if agent_db.exists():
        ...
        encrypted_path = ENCRYPTED_DIR / "agent.db.enc"
        with open(encrypted_path, 'wb') as f:
            f.write(encrypted)

        files_encrypted.append(str(agent_db.relative_to(AGENT_DIR)))
        agent_db.unlink()
        print(f"✓ Encrypted: {agent_db}")
...
def decrypt_vault(passphrase: str = None):
    ...
    for enc_file in ENCRYPTED_DIR.glob("*.enc"):
        ...
        if original_name == "agent.db":
            with open(enc_file, 'rb') as f:
                encrypted = f.read()
            decrypted_content = decrypt_bytes(encrypted, key)
            original_path = AGENT_DIR / "agent.db"
            ...
            original_path = AGENT_DIR / original_rel
            original_path.parent.mkdir(parents=True, exist_ok=True)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="{{AGENT_NAME}} Vault Encryption")
