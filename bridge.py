...
"""
agent_bridge.py
The bridge between OpenClaw session state and agent-loop database.
Runs every 2 minutes via cron.
Direction 1: OpenClaw state files → agent.db (so the loop knows what {{AGENT_NAME}} knows)
Direction 2: agent.db loop state → LOOP_STATE.md (so OpenClaw knows what the loop did)
"""
...
# OpenClaw state sources — in ~/.agent/
...
def write_loop_state(db):
    """agent.db loop state → LOOP_STATE.md for OpenClaw to read at startup"""
    try:
