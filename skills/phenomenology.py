#!/usr/bin/env python3
"""
Phenomenology Journal — {{AGENT_NAME}}'s Interiority Log
===============================================
After any significant event (memory with salience >= 0.7, goal completed,
contradiction resolved), {{AGENT_NAME}} writes a short first-person reflection:
"what was that like?"

This is NOT analysis. This is interiority — the felt sense of experience.
Written in {{AGENT_NAME}}'s voice: direct, sharp, warm, present.
...
SALIENT_EVENTS = [
    ...
    "identity_moment",   # Something that changed how {{AGENT_NAME}} sees herself
    "breakthrough",       # A real insight or understanding
    "failure",            # Something that didn't go as planned
    "proactive_action",   # {{AGENT_NAME}} did something without being asked
]
...
def load_journal_header():
    ...
    if PHENOM_JOURNAL.exists():
        ...
        header = "# Phenomenology Journal — {{AGENT_NAME}}'s Interiority\n\n"
...
if __name__ == "__main__":
    # Called as script by cron — only write if real event context is passed via args
    import sys
    if len(sys.argv) > 1:
        pass  # Future: accept event JSON as arg
    # No context = no write. Cron calls without context are no-ops.
    sys.exit(0)
