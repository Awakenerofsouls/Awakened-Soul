"""
Tests for study activity (Activity Port 2).
...
from heartbeat_activities.study import run, _parse_interests
...
def test_returns_complete_on_valid_run(interests_file, state, monkeypatch):
    """Happy path — mock generate at the study module level."""
    def fake_generate(prompt, **kw):
        assert "chaos theory" in prompt
        return "Chaos theory shows tiny changes compound into large effects."

    import heartbeat_activities.study as study_module
    monkeypatch.setattr(study_module, "generate", fake_generate)
...
def test_weighted_pick_favors_due_interest(interests_file, state, monkeypatch):
    """
    When all topics have been studied, the oldest-due one wins.
    """
    state["tick_count"] = 10
    state["last_studied"] = {
...
def test_continuation_prompt_injected(interests_file, state, monkeypatch):
    """When state has continuation_of=study, prior content is injected."""
    state["continuation_of"] = "study"
    state["prior_study_content"] = "Chaos theory shows non-linear growth."
