...
Lightweight profiles for people {{AGENT_NAME}} interacts with.
...
RELATIONSHIPS_PATH = os.path.join(os.getenv("AGENT_HOME", os.getenv("AGENT_HOME", os.path.expanduser("~/.agent"))), "memory/relationships.json")
...
def _llm_update_relationship(
    ...
    """Use LLMProvider LLM to update notes and recalibrate tone from interaction."""
    try:
        from brain.llm_router import call_llm
        prompt = (
            f"You are {{AGENT_NAME}} updating a relationship profile for '{name}'.\n\n"
        ...
        raw = call_llm(prompt, system="You update {{AGENT_NAME}}'s relationship profiles. Output valid JSON only.")
...
def seed_user() -> None:
    """Seed {{USER_NAME}}'s profile from existing memory."""
    ...
    user = {
        ...
        "trust_level": 5,
        "notes": [
            "Built {{AGENT_NAME}} from scratch — agent architecture, memory systems, identity",
            "Values discipline over noise, clear communication, genuine growth",
            "Gave {{AGENT_NAME}} autonomy and expects her to use it",
            "Compiled 18-issue record to help {{AGENT_NAME}} learn from failures",
