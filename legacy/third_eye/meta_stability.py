"""
MetaStability — ThirdEyeState foundation for Nexus {{AGENT_NAME}} 18.0
Thin persistent state container. All other Third Eye systems read from this.
Lives in ContextSurvival — not a separate persistence mechanism.

Rules:
- Reads compressed summaries only, never raw full-resolution history
- external_anchor_hash grounds coherence against crown_jewels (prevents Observer Illusion)
- All state decays — nothing accumulates indefinitely
- No signals emitted from here — this is read-only state for other systems
"""
...
class MetaStability:
    ...
    def __init__(self, db_path: Optional[Path] = None):
        ...
        self.contradiction_pressure: float = 0.0  # key output — PreConsciousSurfacer reads this
    ...
    def tick(self, pirp_context: dict) -> dict:
        """
        Called every brain tick. Updates tension, coherence, contradiction pressure.
        pirp_context keys used: contradictions, belief_stability, identity_coherence,
        layer6_self_model, layer8_narrative
        """
        ...
        # Contradiction pressure — primary output for PreConsciousSurfacer
        raw_pressure = min(1.0, len(contradictions) * 0.15 +
                           layer6.get("conflict_score", 0.0) * 0.5)
        self.contradiction_pressure = round(
            (1 - EMA_ALPHA) * self.contradiction_pressure + EMA_ALPHA * raw_pressure, 4
        )
        ...
        if self.external_anchor_hash:
            ...
            # Drift = simple character-level mismatch ratio (cheap, no ML needed)
