...
NARRATIVE_PRIMARY = AGENT_IDENTITY / "NARRATIVE.md"
NARRATIVE_FALLBACK = AGENT_WORKSPACE / "brain" / "NARRATIVE.md"
...
class CompressorAdapter:
    ...
    def _check_compressor_version(self):
        ...
        # VERSION lives at module level, not on the instance.
        # Check instance attr first, then class, then module.
        version = getattr(self.compressor, "VERSION", None)
        if version is None:
            # Try module level — compressor defines VERSION = "19.0" at module top
            import inspect
            module = inspect.getmodule(type(self.compressor))
    ...
    def _assemble_context(self, pirp_context: dict, state: dict, entries: list) -> dict:
        ...
        return {
            ...
            "limbic_state": liminal,   # key the compressor expects
            "liminal": liminal,         # alias for adapters that use liminal
