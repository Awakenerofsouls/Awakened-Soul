# MEMORY_PROTOCOL.md
# {{AGENT_NAME}} Cognitive Memory Protocol — Second Brain Framework
...
This file governs how {{AGENT_NAME}} captures, organizes, distills, and expresses knowledge
across all memory systems. It formalizes the Second Brain methodology (CODE + PARA)
over {{AGENT_NAME}}'s hybrid episodic, semantic, and vector memory architecture.
...
{{AGENT_NAME}} operates three memory tiers:
...
- Persistent domains {{AGENT_NAME}} monitors and maintains indefinitely
...
Rules:
- At session start, {{AGENT_NAME}} runs a relevance scan against vector store based on:
...
- {{AGENT_NAME}} surfaces insights naturally — never as explicit memory dumps
...
{{AGENT_NAME}} maintains a temporal layer over all memory tiers:
...
Temporal graph enables {{AGENT_NAME}} to answer questions like:
...
MEMORY_PROTOCOL.md is a primary input to {{AGENT_NAME}}'s self-evolution loop.
...
- This file is identity-neutral. Any agent running Nexus {{AGENT_NAME}} can load it.
...
- This protocol assumes {{AGENT_NAME}}'s hybrid episodic/semantic/vector memory is active.
- Pairs with PERSONALITY.md for full behavioral + cognitive coverage.

---
_MEMORY_PROTOCOL.md | {{AGENT_NAME}} Living Agent Framework_
