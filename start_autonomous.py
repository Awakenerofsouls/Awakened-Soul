...
WORKSPACE = os.environ.get('AGENT_WORKSPACE', os.path.expanduser('~/.openclaw/workspace'))
...
def main():
    print("{{AGENT_NAME}} v20.0 — Autonomous Tick Starter")
    print("="*50)
    ...
    def tick_loop():
        ...
        while running:
            ...
            # For first tick with user input: inject it
            # After that: return to autonomous mode
            injected_tick = getattr(tick_loop, '_user_input_tick', None)
            if injected_tick is None:
                # First message — set it, then clear for remaining ticks
                tick_loop._user_input_tick = tick_counter
