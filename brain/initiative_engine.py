...
{{AGENT_NAME}}'s Initiative Engine — detects what isn't happening and asks why.
...
self-directed goal. Goals are {{AGENT_NAME}}'s — system generates candidates, {{AGENT_NAME}}
...
- Curiosity engine surfaces questions to {{AGENT_NAME}}, never to the user directly without {{AGENT_NAME}}'s choice.
...
def generate_goals_from_absence(absence_signals: list = None) -> list:
    ...
    Goals are {{AGENT_NAME}}'s — system generates candidates, {{AGENT_NAME}} implicitly acts on them.
