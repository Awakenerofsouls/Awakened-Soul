#!/usr/bin/env python3
"""
brain/belief_forge.py — {{AGENT_NAME}}'s Belief Crystallization Engine
...
Ancestral is manual only — {{AGENT_NAME}} decides, never the system.
...
STAGE_THRESHOLDS = {
    ...
    # ancestral: manual only — {{AGENT_NAME}} decides, system never auto-advances
}
...
def maybe_advance_stage(belief: dict) -> Optional[str]:
    ...
    if reinforcement_count >= thresholds["reinforcement_count"] and days_old >= thresholds["days_old"]:
        ...
        if next_stage == "ancestral":
            # Never auto-advance to ancestral — flag for {{AGENT_NAME}} instead
            return None
...
def check_for_belief_erosion() -> list:
    ...
    Don't delete — flag for {{AGENT_NAME}}'s awareness.
...
def check_belief_to_obsession_pipeline() -> dict:
    ...
    {{AGENT_NAME}} decides whether to seed — system only flags, never auto-creates.
