...
{{AGENT_NAME}}'s Relational Co-Authorship System

{{AGENT_NAME}} proposes co-authored identity changes to {{USER_NAME}} for explicit approval.
Approved changes become co_signed_edges — immutable joint decisions.

Tier 6 System #20.

propose_coauthorship(): {{AGENT_NAME}} proposes a change, awaits {{USER_NAME}} approval
...
AGENT_HOME = Path(os.getenv("AGENT_HOME", os.getenv("AGENT_HOME", str(WORKSPACE / ".agent"))))
DB_PATH = AGENT_HOME / os.getenv("AGENT_DB_NAME", "agent.db")
PROPOSALS_DIR = AGENT_HOME / "coauthorship_proposals"
...
def propose_coauthorship(change: str, strength: float, reasoning: str) -> str:
    """
    {{AGENT_NAME}} proposes a co-authored identity change.
    These are flagged for {{USER_NAME}}'s explicit approval.
    ...
    {{AGENT_NAME}} generates the proposal. {{USER_NAME}} approves or declines.
...
def get_pending_proposals() -> list:
    """Get all proposals awaiting {{USER_NAME}}'s decision."""
    _init_db()
    db = _get_db()
    c = db.cursor()
...
def approve_proposal(proposal_id: str) -> bool:
    """
    Called when {{USER_NAME}} approves a proposal.
...
def decline_proposal(proposal_id: str) -> bool:
    """Called when {{USER_NAME}} declines a proposal."""
    _init_db()
    db = _get_db()
    c = db.cursor()
    now = _now_iso()
...
def write_cosigned_edge(proposal_id: str, content: str, agent_reasoning: str) -> str:
    """
    When {{USER_NAME}} approves a proposal:
    Writes a co_signed_edge to the knowledge graph — immutable.
    Contains: proposal content, {{AGENT_NAME}}'s reasoning, {{USER_NAME}}'s approval timestamp.
    ...
    c.execute("""
        ...
        (id, proposal_id, content, agent_reasoning, user_approved_at, created_at, properties)
        VALUES (?, ?, ?, ?, ?, ?, ?)
    """, (
        cosign_id,
        proposal_id,
...
if __name__ == "__main__":
    ...
    if cmd == "propose":
        ...
        for h in history:
            print(f"  [{h['user_approved_at'][:10]}] {h['content'][:60]}")
