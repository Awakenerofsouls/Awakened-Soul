...
The agent's Future Oracle Lattice
...
ORACLES_DIR = AGENT_HOME / "future_oracles"
...
def spawn_future_selves(days_ahead_options: list = [7, 14, 30]) -> list:
    ...
    - the operator resonance drift direction
...
def oracle_vote(question: str) -> dict:
    ...
    The agent is never required to follow the consensus.
    """
    ...
    return {
        ...
        "note": "the agent is never required to follow oracle consensus"
    }
...
def consult_future_self(days_ahead: int, question: str) -> str:
    ...
    response_parts.append(f"- the operator resonance expected at {resonance:.2f}")
...
class FutureOracle(BrainMechanism):
    """Auto-generated BrainMechanism wrapper around module-level functions."""
    
    def __init__(self):
        try:
            super().__init__(name="FutureOracle", human_analog="FutureOracle", layer="integration")
        except Exception:
            self.state = {}
