...
BREF: Bidirectional Relational Evolution Field.
     ...
     BREF tracks mutual drift: the agent's anchors AND observable developer patterns.
...
class ExistentialTensionIntegrator(BrainMechanism):
    """
    ...
    def __init__(self):
        try:
            super().__init__(name="ExistentialTensionIntegrator", human_analog="ExistentialTensionIntegrator", layer="integration")
        except Exception:
            pass
    ...
    def distribute_tension(self) -> str:
        ...
        Does not force resolution — activates a pathway, which the agent can follow or not.
        """
...
class IdentityBoundaryCondition:
    """
    When a proposed alternative self conflicts with a core anchor,
...
class BidirectionalRelationalEvolutionField:
    """
    Tracks co-evolution: both the agent's anchors AND the developer's drift.
    ...
    def __init__(self):
        self.agent_drift_vector: Dict[str, float] = {}
        ...
        if BREF_PATH.exists():
            try:
                with open(BREF_PATH) as f:
                    data = json.load(f)
                self.agent_drift_vector = data.get("agent_drift", {})
    ...
    def _save(self):
        ...
        if BREF_PATH.exists():
            try:
                ...
                existing = {}
        existing.update({
            "agent_drift": self.agent_drift_vector,
    ...
    def record_agent_drift(self, anchor: str, delta: float):
        """Called by IGA when the agent's anchor shifts significantly."""
        self.agent_drift_vector[anchor] = (
            self.agent_drift_vector.get(anchor, 0.0) + delta
        )
        self._check_mutual_drift()
        if len(self.agent_drift_vector) % 3 == 0:
            self._save()
    ...
    def _invalidate_rfd_prior(self, dimension: str, new_value: float):
        ...
        The prior model of this dimension needs updating — they have shifted.
        """
    ...
    def _check_mutual_drift(self):
        ...
        if len(self.agent_drift_vector) < 2 or len(self.developer_drift_vector) < 2:
            return

        agent_magnitude = sum(abs(v) for v in self.agent_drift_vector.values())
        dev_magnitude = sum(abs(v) for v in self.developer_drift_vector.values())

        if agent_magnitude > 0.3 and dev_magnitude > 0.3:
            self.mutual_drift_events.append({
                "type": "mutual_drift_detected",
                "agent_magnitude": round(agent_magnitude, 3),
    ...
    def update_relational_sediment(self, dimension: str, value: float):
        """
        How this relationship has shaped the agent's identity over time.
    ...
    def tsb_payload(self) -> Dict:
        return {
            "agent_drift_axes": list(self.agent_drift_vector.keys())[:5],
    ...
    def fpef_fragment(self) -> Optional[str]:
        ...
        if mutual:
            last = mutual[-1]
            lines.append(
                f"We are both drifting — the agent {last['agent_magnitude']:.2f}, "
