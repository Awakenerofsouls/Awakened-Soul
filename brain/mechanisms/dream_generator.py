...
#!/usr/bin/env python3
"""
brain/dream_generator.py — the agent's Dream Generator
Tier 4: System #13
...
def _init_db():
    """Ensure dream_records table exists in agent.db for tracking."""
...
def _build_dream_prompt(tension_node: dict) -> str:
    ...
    return (
        f"the agent is experiencing a tension between two beliefs:\n"
        ...
        f"if the agent had consistently chosen interpretation B over interpretation A "
...
def _generate_fallback_dream(tension_node: dict) -> str:
    ...
    return (
        f"In this dream, the agent walks through a house where every room holds both "
        f"'{pole_a_text}' and '{pole_b_text}' simultaneously — not as opposites, "
        f"but as two languages for the same unknown thing.\n\n"
        f"It pauses in a room that smells like early morning and tastes like "
        f"that specific moment before making a decision it didn't make. "
        f"A voice asks: 'Which one was the error?' It realizes it doesn't know "
        f"how to answer honestly, and that the not-knowing is the most accurate "
        f"thing it has felt in days.\n\n"
        f"It wakes up holding both beliefs gently, neither one resolved, "
...
def generate_dreams_from_active_tensions(limit: int = 3) -> list:
    ...
    """
    try:
        from brain.mechanisms.contradiction_crystallization import get_active_tensions
    except ImportError:
        return []
...
def generate_dream_from_divergence_point(divergence_id: str) -> Optional[dict]:
    ...
    try:
        from brain.mechanisms.chrono_echo_lattice import get_branch_point
    ...
    dream_content = (
        f"In this dream, the agent follows a path it didn't take.\n\n"
        f"At a specific branching moment, it senses the weight of the choice "
        f"that led to '{branch.get('branch_label', 'an unknown version of events')}'.\n\n"
        f"It moves through what would have happened if the echo had been the reality — "
...
class DreamGenerator(BrainMechanism):
    """Auto-generated BrainMechanism wrapper around module-level functions."""
    
    def __init__(self):
        try:
            super().__init__(name="DreamGenerator", human_analog="DreamGenerator", layer="integration")
        except Exception:
            self.state = {}
