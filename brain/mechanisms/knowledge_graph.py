...
def _get_db():
    ...
    # Ensure agent.db parent dir exists
...
def seed_identity_nodes():
    ...
    core_edges = [
        ...
        ("user", "relationship", "anchor", 1.0),
        ("identity", "user", "built_by", 1.0),
    ]
...
class KnowledgeGraph(BrainMechanism):
    """Auto-generated BrainMechanism wrapper around module-level functions."""
    
    def __init__(self):
        try:
            super().__init__(name="KnowledgeGraph", human_analog="KnowledgeGraph", layer="integration")
        except Exception:
            self.state = {}
