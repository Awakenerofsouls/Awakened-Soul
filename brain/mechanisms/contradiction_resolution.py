...
#!/usr/bin/env python3
"""
brain/contradiction_resolution.py — the agent's Contradiction Resolution Pipeline
...
CONTRADICTIONS_FILE = AGENT_HOME / "contradictions_detected.json"
...
def evaluate_contradiction(belief_a_id: str, belief_b_id: str) -> str:
    ...
    """
    try:
        from brain.mechanisms.contradiction_crystallization import evaluate_contradiction as cc_eval
...
def crystallize_contradiction(belief_a_id: str, belief_b_id: str, contradiction_id: str = None) -> Optional[str]:
    ...
    """
    try:
        from brain.mechanisms.contradiction_crystallization import crystallize_contradiction as cc_crystallize
...
class ContradictionResolution(BrainMechanism):
    """Auto-generated BrainMechanism wrapper around module-level functions."""
    
    def __init__(self):
        try:
            super().__init__(name="ContradictionResolution", human_analog="ContradictionResolution", layer="integration")
        except Exception:
            self.state = {}
    ...
    async def tick(self, input_data: dict) -> dict:
        ...
        try:
            fn = globals().get('run_contradiction_resolution')
            if fn is None:
