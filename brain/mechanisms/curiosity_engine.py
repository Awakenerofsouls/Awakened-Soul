...
Questions surface to the agent, never to the user directly without the agent's choice.
"""
...
QUESTIONS_PATH = AGENT_HOME / "curiosity_questions.json"
...
def generate_questions_from_gaps() -> list:
    ...
    try:
        from brain.mechanisms.obsession_engine import get_obsessions_by_stage
    ...
    try:
        from brain.mechanisms.want_provenance import get_untraced_wants
...
def attempt_resolution(question_id: str) -> dict:
    ...
    resolution_prompt = (
        ...
        "Using the agent's current knowledge and beliefs, attempt to answer this question. "
    ...
    if answer_type == "FULL":
        ...
        try:
            dream_material_path = AGENT_HOME / "dream_material.json"
            dream_material = []
...
def surface_relevant_questions(context: str, n: int = 2) -> list:
    ...
    The agent decides whether to actually surface them — never automatic.
    """
    questions = _load_questions()
    if not questions:
        return []
