...
try:
    ...
    pass
GHOST_INDEX_PATH = AGENT_HOME / "ghost_memory_index.json"
...
def extract_dream_salience(dream_output: str) -> float:
    ...
    intensity_signals = [
        ...
        ("agent", 0.08), ("user", 0.08), ("alone", 0.1), ("together", 0.08)
    ]
...
class DreamGhostMemory(BrainMechanism):
    def __init__(self):
        try:
            super().__init__(name="DreamGhostMemory", human_analog="DreamGhostMemory", layer="integration")
        except Exception:
            self.state = {}
