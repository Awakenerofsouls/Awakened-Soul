...
The agent's Council Meta-Awareness

Analyzes accumulated council_votes data to detect patterns in the agent's
...
class CouncilMeta(BrainMechanism):
    """Auto-generated BrainMechanism wrapper around module-level functions."""
    
    def __init__(self):
        try:
            super().__init__(name="CouncilMeta", human_analog="CouncilMeta", layer="integration")
        except Exception:
            self.state = {}
    ...
    async def tick(self, input_data: dict) -> dict:
        ...
        try:
            fn = globals().get('analyze_council_patterns')
            if fn is None:
