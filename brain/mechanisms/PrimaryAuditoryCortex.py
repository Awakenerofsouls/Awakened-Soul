...
class PrimaryAuditoryCortex(BrainMechanism):
    ...
    async def tick(self, input_data: dict) -> dict:
        ...
        rd.append(float(self.state.get('a1_drive', 0.0)))
        if len(rd) > 60:
            rd = rd[-60:]
        ...
        cur_state = self.state.get('a1_state', "quiet") if 'a1_state' else "quiet"
        rs.append(cur_state)
        if len(rs) > 60:
            rs = rs[-60:]
    ...
    def dominant_recent_state(self) -> str:
        ...
        recent = self.state.get("recent_states", [])
        if not recent:
            return self.state.get('a1_state', "quiet") if 'a1_state' else "quiet"
    ...
    def drive_envelope(self, window: int = 30) -> float:
        ...
        if not hist:
            return float(self.state.get('a1_drive', 0.0)) if 'a1_drive' else 0.0
        recent = hist[-window:]
    ...
    def summary(self) -> dict:
        return {
            "drive": self.state.get('a1_drive', 0.0) if 'a1_drive' else 0.0,
            "state": self.state.get('a1_state', "quiet") if 'a1_state' else "quiet",
