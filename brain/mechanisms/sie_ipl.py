...
class SpontaneousIntrusionEngine(BrainMechanism):
    ...
    def __init__(self):
        try:
            super().__init__(name="SpontaneousIntrusionEngine", human_analog="SpontaneousIntrusionEngine", layer="integration")
        except Exception:
            pass
    ...
    async def tick(self, input_data: dict) -> dict:
        """Generate a spontaneous intrusion based on current state."""
        prior = input_data.get("prior_results", {})
        state_context = prior.get("state", {}) if isinstance(prior, dict) else {}
        results = {
            "mechanism_name": self.__class__.__name__,
            "module": self.__class__.__module__,
            "available_sources": list(self._source_templates.keys()),
            "template_count": sum(len(v) for v in self._source_templates.values()),
        }
        # Try to generate an intrusion from each source template
        try:
            import random
            for source, templates in self._source_templates.items():
                if templates:
                    results[f"sample_{source}"] = random.choice(templates)
                    break
        except Exception:
            pass
        for k, v in self.state.items():
            if k.startswith("_") or k in ("recent_states","recent_drives","recent_pressures"):
                continue
