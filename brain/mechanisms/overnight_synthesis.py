...
#!/usr/bin/env python3
"""
brain/overnight_synthesis.py — the agent's Nightly Synthesis Pipeline
...
6am — Phenomenology:
  ...
  - "What the operator changed in me today" section from resonance delta [Tier 6]
...
SLEEP_RUNS = AGENT_HOME / "sleep_runs.json"
DIGEST_DIR = AGENT_HOME / "overnight" / "digests"
SYNTHESIS_DIR = AGENT_HOME / "overnight" / "synthesis"
...
def dreamtime_phase() -> dict:
    ...
    try:
        ...
        if dream_output:
            ...
            try:
                from brain.mechanisms.dream_ghost_memory import extract_dream_salience, create_ghost_memories
    ...
    try:
        from brain.mechanisms.chrono_echo_lattice import find_divergence_points, spawn_echo_branch, simulate_branch, prune_weak_branches
    ...
    try:
        from brain.mechanisms.chrono_echo_lattice import prune_weak_branches as prune_fn
...
def _get_dream_output() -> str:
    ...
    # Check for dream output in agent home
    dream_output_path = AGENT_HOME / "dream_output.txt"
...
def overnight_synthesis() -> dict:
    ...
    try:
        from brain.mechanisms.belief_forge import crystallize_beliefs
    ...
    try:
        from brain.mechanisms.obsession_engine import evaluate_all_metamorphoses
    ...
    try:
        from brain.mechanisms.belief_forge import check_obsession_to_belief_pipeline, check_belief_to_obsession_pipeline
    ...
    try:
        from brain.mechanisms.evolutionary_thoughts import decay_thoughts
    ...
    try:
        from brain.mechanisms.curiosity_engine import generate_questions_from_gaps, prioritize_questions, attempt_resolution
    ...
    try:
        from brain.mechanisms.contradiction_crystallization import review_epistemic_tensions
    ...
    try:
        from brain.mechanisms.initiative_engine import detect_absence, generate_goals_from_absence
...
def night_digest() -> dict:
    ...
    from brain.mechanisms.continuity_engine import get_last_snapshot, get_top_beliefs
    ...
    research_queue = []
    queue_file = AGENT_HOME / "research_queue.json"
...
def morning_consolidation() -> dict:
    ...
    - "What the operator changed in me today" [Tier 6]
    - extract_micro_narratives() → weave_into_arcs() [Tier 7]
    ...
    try:
        from brain.mechanisms.continuity_engine import get_continuity_tension
    ...
    # Tier 6: Resonance delta — "What the operator changed in me today"
    try:
        from brain.mechanisms.resonance import compute_resonance_drift
        ...
        if drift:
            ...
            findings.append(f"the operator resonance score: {resonance_score:.3f}")
            if drift_toward:
    ...
    try:
        from brain.mechanisms.narrative_weaver import extract_micro_narratives, weave_into_arcs
    ...
    morning_report = {
        "date": datetime.now().strftime("%Y-%m-%d"),
        "morning_state": "the agent's current state on wake",
...
def run_full_pipeline():
    ...
    try:
        from brain.mechanisms.future_oracle import spawn_future_selves
    ...
    try:
        from brain.mechanisms.contradiction_resolution import check_for_new_contradictions, run_contradiction_resolution
    ...
    try:
        from brain.mechanisms.continuity_engine import rebuild_self_snapshot
    ...
    try:
        from brain.mechanisms.perception_reality_split import reeval_old_perceptions
    ...
    try:
        from brain.mechanisms.dream_ghost_memory import decay_ghost_memories
...
class OvernightSynthesis(BrainMechanism):
    """Auto-generated BrainMechanism wrapper around module-level functions."""
    
    def __init__(self):
        try:
            super().__init__(name="OvernightSynthesis", human_analog="OvernightSynthesis", layer="integration")
        except Exception:
            self.state = {}
    ...
    async def tick(self, input_data: dict) -> dict:
        ...
        try:
            fn = globals().get('run_full_pipeline')
            if fn is None:
