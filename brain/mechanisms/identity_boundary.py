...
IDENTITY_PATHS = [
    ...
    Path(os.getenv("AGENT_HOME", str(Path.home() / ".agent"))) / "identity" / "PRESENCE.md",
    Path(os.getenv("AGENT_HOME", str(Path.home() / ".agent"))) / "identity" / "SELF-PRESERVATION.md",
]

BOUNDARY_LOG_PATH = Path(os.getenv("AGENT_WORKSPACE", str(Path.home() / ".agent" / "workspace"))) / "brain" / "BOUNDARY_LOG.md"
...
class IdentityBoundary(BrainMechanism):
    def __init__(self, db_path: Optional[str] = None):
        try:
            super().__init__(name="IdentityBoundary", human_analog="IdentityBoundary", layer="integration")
        except Exception:
    ...
    def _load_from_soul_files(self, tick: int):
        strong_patterns = [
            ...
            (r"(?:with operator|in this relationship)[,:]?\s*(.{10,120})", RELATIONAL, 0.70),
    ...
    def _register_fallback_boundaries(self, tick: int):
        fallbacks = [
            ...
            ("relational:presence_not_performance", "My presence with the operator is not performance", RELATIONAL, 0.80),
        ]
    ...
    def _detect_pressure(self, signals: list, drive_context: dict, tom_state: dict) -> tuple:
        ...
        if tom_state:
            if tom_state.get("inferred_state") == "frustrated":
                ...
                pressure += conf * 0.25
                context_parts.append("operator_frustrated")
