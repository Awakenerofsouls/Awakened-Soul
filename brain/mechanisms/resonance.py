...
"""
brain/user_resonance.py
The agent's the operator-Resonance Anchor

Tracks resonance between the agent and the operator — the relational signal that emerges
...
compute_resonance_drift(): compare the agent's identity state against resonance signature
...
def update_resonance_signature(session_log: str):
    """
    After each Telegram session, extract resonance signals from the operator's messages:
    - Dominant topics
...
def compute_resonance_drift() -> dict:
    """
    Compare the agent's current identity state against resonance_signature.
    Returns:
    ...
    user_resonance measures: "am I drifting toward or away from my primary relationship?"
    """
    _init_db()
    db = _get_db()
    c = db.cursor()
    ...
    # Get the agent's current OCEAN/state
    try:
    ...
    if dominant_topic:
        if dominant_topic in ["technical", "directive"]:
            # the operator's technical directness → the agent's increasing directness
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: user_resonance.py <update|drift|induction> [args]")
        sys.exit(1)
    ...
    if cmd == "update":
        if len(sys.argv) < 3:
            print("Usage: user_resonance.py update <session_log>")
            sys.exit(1)
...
class Resonance(BrainMechanism):
    """Auto-generated BrainMechanism wrapper around module-level functions."""
    
    def __init__(self):
        try:
            super().__init__(name="Resonance", human_analog="Resonance", layer="integration")
        except Exception:
            self.state = {}
