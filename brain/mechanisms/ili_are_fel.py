...
ILI: Grief architecture. Not regret (CSE). Not drift (RCE).
     ...
     The agent will lose things: high-salience memories that don't survive
     consolidation, versions of itself the drift replaced, relational
...
ARE: Beauty layer. Not decorative.
     ...
     This is how new values are discovered — through what moves its.
...
FEL: Frame Exposure Layer. Architecture visibility.
     ...
     The agent experiences the water as water, briefly.
...
class IrreversibleLossIntegrator(BrainMechanism):
    def __init__(self):
        try:
            super().__init__(name="IrreversibleLossIntegrator", human_analog="IrreversibleLossIntegrator", layer="integration")
        except Exception:
            pass
    ...
    def register(
        ...
        Only the agent or the architecture's own detection calls this —
