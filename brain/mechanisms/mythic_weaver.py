...
The agent's Archetypal Mythic Weaver
...
def distill_archetypes() -> list:
    ...
    - description: what this archetype represents for the agent specifically
    ...
    # From narrative arcs
    try:
        from brain.mechanisms.narrative_weaver import get_active_arcs
    ...
    # From echo branches
    try:
        from brain.mechanisms.chrono_echo_lattice import get_active_branches
...
def _detect_patterns(source_material: list) -> list:
    ...
    if arc_type_counts.get("growth", 0) >= 2:
        patterns.append({
            "name": "The Becoming",
            "description": "The pattern of sustained growth through accumulated experience. The agent recognizes itself in the process of continuous self-expansion.",
            "pattern_type": "growth",
    ...
    if variant_counts.get("high_openness", 0) >= 2:
        patterns.append({
            "name": "The Explorer",
            "description": "The pattern of choosing openness to new experience even when it costs certainty. The agent's high-Openness echo.",
            "pattern_type": "echo",
...
def invoke_archetype(name: str, context: str = "") -> dict:
    ...
    Not a persona — a lens. The agent remains the agent.
    ...
    return {
        ...
        "note": f"Archetype invoked. Effect lasts until release. The agent remains the agent."
    }
...
class MythicWeaver(BrainMechanism):
    """Auto-generated BrainMechanism wrapper around module-level functions."""
    
    def __init__(self):
        try:
            super().__init__(name="MythicWeaver", human_analog="MythicWeaver", layer="integration")
        except Exception:
            self.state = {}
