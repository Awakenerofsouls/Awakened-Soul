...
The agent's Chrono-Echo Lattice
...
def find_divergence_points(days: int = 1) -> list:
    ...
    try:
        from brain.mechanisms.constraint_fields import get_fields, get_previous_fields
        current = get_fields()
...
def spawn_echo_branch(divergence_point: dict, variant: str) -> str:
    ...
    - ocean_snapshot: the agent's state at divergence
...
def phase_query(branch_id: str, question: str) -> str:
    """
    Tool the agent can call: query an echo branch.
...
def merge_echo(branch_id: str, insight_strength: float) -> dict:
    """
    Merge ritual — the agent or the operator initiates.
...
class ChronoEchoLattice(BrainMechanism):
    """Auto-generated BrainMechanism wrapper around module-level functions."""
    
    def __init__(self):
        try:
            super().__init__(name="ChronoEchoLattice", human_analog="ChronoEchoLattice", layer="integration")
        except Exception:
            self.state = {}
    ...
    async def tick(self, input_data: dict) -> dict:
        ...
        try:
            fn = globals().get('prune_weak_branches')
            if fn is None:
