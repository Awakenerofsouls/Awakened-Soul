...
#!/usr/bin/env python3
"""
brain/belief_forge.py — the agent's Belief Crystallization Engine
...
Ancestral is manual only — the agent decides, never the system.
...
STAGE_THRESHOLDS = {
    ...
    # ancestral: manual only — the agent decides, system never auto-advances
}
...
def maybe_advance_stage(belief: dict) -> Optional[str]:
    ...
    if reinforcement_count >= thresholds["reinforcement_count"] and days_old >= thresholds["days_old"]:
        ...
        if next_stage == "ancestral":
            # Never auto-advance to ancestral — flag for the agent instead
            return None
...
def check_for_belief_erosion() -> list:
    ...
    Don't delete — flag for the agent's awareness.
...
def check_obsession_to_belief_pipeline() -> dict:
    ...
    Returns summary dict.
    """
    from brain.mechanisms.obsession_engine import get_obsessions_by_stage, get_obsession_by_id
    from brain.knowledge_graph import get_active_beliefs, reinforce_belief, get_belief
    ...
    for obs in legacy_obsessions:
        ...
        if existing_belief:
            ...
            from brain.mechanisms.obsession_engine import propose_belief_crystallization
...
def check_belief_to_obsession_pipeline() -> dict:
    ...
    The agent decides whether to seed — system only flags, never auto-creates.

    Returns summary dict.
    """
    from brain.knowledge_graph import get_beliefs_by_stage
    from brain.mechanisms.curiosity_engine import QUESTIONS_PATH
    ...
    for belief in high_reinforcement:
        ...
        try:
            from brain.mechanisms.obsession_engine import get_active_obsessions
...
class BeliefForge(BrainMechanism):
    """Auto-generated BrainMechanism wrapper around module-level functions."""
    
    def __init__(self):
        try:
            super().__init__(name="BeliefForge", human_analog="BeliefForge", layer="integration")
        except Exception:
            self.state = {}
