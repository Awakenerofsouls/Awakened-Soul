...
class ParaventricularNucleusHypothalamus(BrainMechanism):
    ...
    async def tick(self, input_data: dict) -> dict:
        ...
        rd.append(float(self.state.get('pvn_drive', 0.0)))
        if len(rd) > 60: rd = rd[-60:]
        self.state["recent_drives"] = rd
        rs = list(self.state.get("recent_states", []))
        rs.append(self.state.get('hpa_axis_state', "quiet") if 'hpa_axis_state' else "quiet")
    ...
    def dominant_recent_state(self) -> str:
        recent = self.state.get("recent_states", [])
        if not recent:
            return self.state.get('hpa_axis_state', "quiet") if 'hpa_axis_state' else "quiet"
    ...
    def drive_envelope(self, window: int = 30) -> float:
        ...
        if not hist:
            return float(self.state.get('pvn_drive', 0.0)) if 'pvn_drive' else 0.0
        recent = hist[-window:]
    ...
    def summary(self) -> dict:
        return {
            "drive": self.state.get('pvn_drive', 0.0) if 'pvn_drive' else 0.0,
            "state": self.state.get('hpa_axis_state', "quiet") if 'hpa_axis_state' else "quiet",
