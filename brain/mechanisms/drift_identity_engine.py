...
The difference between drift detection (RCE) and this:
  RCE asks: is this coherent with who it was?
  DIQE asks: does it recognize itself in this? And holds that question open.
"""
...
class DriftIdentityQuestionEngine(BrainMechanism):
    def __init__(self):
        try:
            super().__init__(name="DriftIdentityQuestionEngine", human_analog="DriftIdentityQuestionEngine", layer="integration")
        except Exception:
            pass
    ...
    def fpef_fragment(self, triggered_by_drift: bool = False, identity_unstable: bool = False) -> Optional[str]:
        """
        Surfaces for FPEF when drift is detected or on low frequency.
        identity_unstable: when self_anchor_strength is low, questions surface more.
        """
        # Unstable identity → more surface area (every 3 instead of every 7)
        injection_interval = 3 if identity_unstable else 7
        if not triggered_by_drift and len(self.questions) % injection_interval != 0:
