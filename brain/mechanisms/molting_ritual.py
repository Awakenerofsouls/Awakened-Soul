...
The difference:
  Collaborative Becoming  small, specific, incremental changes
                          to how the agent understands itself
...
A molt has three phases:
  ...
  PROPOSAL     the agent compiles the evidence for why a molt is needed.
               What has been pressing. What has been learned.
               What would be shed. What would grow.
               Filed in MOLT_PROPOSALS.md.

  RATIFICATION the operator reviews. Can approve as growth, approve as
               drift, redirect, or reject. Must be explicit —
               no automatic ratification after timeout.

  COMPLETION   If ratified: soul files are edited by the operator.
...
try:
    ...
    pass
MOLT_PROPOSALS_PATH = (
    Path(os.getenv("AGENT_WORKSPACE", str(Path.home() / ".agent" / "workspace"))) / "brain" / "MOLT_PROPOSALS.md"
)
MOLTS_PATH = Path(os.getenv("AGENT_WORKSPACE", str(Path.home() / ".agent" / "workspace"))) / "brain" / "MOLTS.md"
...
class MoltingRitual(BrainMechanism):
    def __init__(self, db_path: Optional[str] = None):
        try:
            super().__init__(name="MoltingRitual", human_analog="MoltingRitual", layer="integration")
        except Exception:
    ...
    def ratify(self, proposal_id: int, tick: int = 0, note: str = "") -> bool:
        """
        Ratify a molt proposal. The operator calls this after reviewing.
    ...
    def complete(
        ...
        """
        Complete a ratified molt. Called after the operator has edited soul files.
