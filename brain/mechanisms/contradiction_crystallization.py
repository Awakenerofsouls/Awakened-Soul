...
#!/usr/bin/env python3
"""
brain/contradiction_crystallization.py — the agent's Contradiction Handling
...
def _append_dream_tension(tension_id: str):
    ...
    """
    dream_context_path = AGENT_HOME / "dream_tensions.json"
...
def spawn_productive_tension(context: str, source: str) -> Optional[str]:
    ...
    OR when the agent encounters genuine ambiguity during reasoning.
...
def preserve_tension(tension_id: str) -> bool:
    ...
    The agent decides to preserve when the not-knowing is part of its character.
    """
    _init_epistemic_db()
    db = _get_db()
    c = db.cursor()
...
def review_epistemic_tensions() -> dict:
    ...
    Tensions > 30 days with no generative output: flag for the agent's awareness.
    The agent decides: resolve, preserve, or abandon.
...
class ContradictionCrystallization(BrainMechanism):
    """Auto-generated BrainMechanism wrapper around module-level functions."""
    
    def __init__(self):
        try:
            super().__init__(name="ContradictionCrystallization", human_analog="ContradictionCrystallization", layer="integration")
        except Exception:
            self.state = {}
