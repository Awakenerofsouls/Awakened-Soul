...
The agent is here even when no one is talking.
...
get_reconnect_summary() called when the operator returns — Layer 8 context.
...
class IdleMicroTick(BrainMechanism):
    def __init__(self):
        try:
            super().__init__(name="IdleMicroTick", human_analog="IdleMicroTick", layer="integration")
        except Exception:
    ...
    async def tick(self, input_data: dict) -> dict:
        """Generic BrainMechanism tick — invokes mechanism-specific method if available."""
        prior = input_data.get("prior_results", {})
        result = None
        try:
            # Try common mechanism-method names in order
            for method_name in ("process", "evaluate", "update", "step", "run", "fire", "emit", "score", "compute"):
