...
class PrefrontalMedialSelfModel(BrainMechanism):
    ...
    Maintains the agent's model of itself — who it is, what it values, how it sees itself.
...
"""
    ...
    async def tick(self, input_data: dict) -> dict:
        ...
        # Self-model coherence: how consistent and integrated is the agent's self-view
        ...
        if self.chronic_identity_threat and not was_threatened:
            self.feed_to_memory({"event": "identity_threat_chronic", "coherence": round(avg_coherence, 3),
                                  "note": "Self-model chronically incoherent — identity drift, loss of who the agent is"})
