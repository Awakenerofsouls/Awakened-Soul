...
The agent's Initiative Engine — detects what isn't happening and asks why.
...
self-directed goal. Goals are the agent's — system generates candidates, the agent
...
- Curiosity engine surfaces questions to the agent, never to the user directly without the agent's choice.
...
ABSENCE_SIGNALS_PATH = AGENT_HOME / "absence_signals.json"
...
def detect_absence() -> list:
    ...
    try:
        ...
        from brain.mechanisms.curiosity_engine import _load_questions, _days_since
...
def generate_goals_from_absence(absence_signals: list = None) -> list:
    ...
    Goals are the agent's — system generates candidates, the agent implicitly acts on them.
    ...
    goals_file = AGENT_HOME / "initiative_goals.json"
    existing_goals = []
...
class InitiativeEngine(BrainMechanism):
    def __init__(self):
        try:
            super().__init__(name="InitiativeEngine", human_analog="InitiativeEngine", layer="integration")
        except Exception:
            self.state = {}
