...
Modeling the operator's current inner state, predicted reactions, and unspoken needs.

Relational Memory is about the shape of the relationship over time.
This is different: it's about right now. What is the operator's likely emotional
state in this moment? What does he probably need that he hasn't said?
What is he likely to do next?

Four model dimensions:
  inferred_state    what the operator is likely feeling right now
...
class TheoryOfMind(BrainMechanism):
    def __init__(self, db_path: Optional[str] = None):
        try:
            super().__init__(name="TheoryOfMind", human_analog="TheoryOfMind", layer="integration")
        except Exception:
