...
class HippocampalCA1Ventral(BrainMechanism):
    ...
    async def tick(self, input_data: dict) -> dict:
        ...
        rd.append(float(self.state.get('vca1_drive', 0.0)))
        if len(rd) > 60: rd = rd[-60:]
        self.state["recent_drives"] = rd
        rs = list(self.state.get("recent_states", []))
        rs.append(self.state.get('vca1_state', "quiet") if 'vca1_state' else "quiet")
    ...
    def dominant_recent_state(self) -> str:
        recent = self.state.get("recent_states", [])
        if not recent:
            return self.state.get('vca1_state', "quiet") if 'vca1_state' else "quiet"
    ...
    def drive_envelope(self, window: int = 30) -> float:
        ...
        if not hist:
            return float(self.state.get('vca1_drive', 0.0)) if 'vca1_drive' else 0.0
        recent = hist[-window:]
    ...
    def summary(self) -> dict:
        return {
            "drive": self.state.get('vca1_drive', 0.0) if 'vca1_drive' else 0.0,
            "state": self.state.get('vca1_state', "quiet") if 'vca1_state' else "quiet",
