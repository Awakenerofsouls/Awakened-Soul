...
class WorkingMemory(BrainMechanism):
    """Lightweight working memory for current session."""

    def __init__(self):
        try:
            super().__init__(name="WorkingMemory", human_analog="WorkingMemory", layer="integration")
        except Exception:
            pass
    ...
    def _score_lenses_heuristic(self, content: str, salience: float,
        ...
        self_relevance = max(0.0, min(1.0, salience * 0.7 + (0.2 if any(t in tags for t in ["self", "identity", "user", "agent"]) else 0)))
        ...
        if user_profile:
            wm.add(
                content=f"[Session start] {OPERATOR_NAME} — last contact: {user_profile.get('last_contact','unknown')}, "
                        ...
                        f"Recent notes: {'; '.join(user_profile.get('notes',[])[:3])}",
                entry_type="context",
                salience=0.85,
                valence=0.3,
                emotional_tags=["user", "relationship"],
