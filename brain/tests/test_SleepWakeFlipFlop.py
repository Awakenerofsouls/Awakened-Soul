...
class TestOutputKeys:
    def test_required_keys_present(self):
        ...
        for key in ['sleep_wake_state', 'vlpo_drive', 'arousal_side_drive', 'sleep_pressure', 'circadian_bias', 'transition_imminent', 'flip_count', 'rem_pattern_active', 'homeostasis_state', 'arousal_index']:
...
class TestNumericBounds:
    ...
    def test_saturated_inputs_stay_bounded(self):
        mech = SleepWakeFlipFlop()
        prior = {dep: {"_saturated": 1.0} for dep in ['ArousalRegulator', 'HistamineArousalBooster', 'CircadianTimer', 'Homeostat', 'OrexinWakePromoter']}
        for _ in range(15):
