"""
Behavioral tests for A2NoradrenergicNTS.

Run:
    pytest brain/tests/test_A2NoradrenergicNTS.py -v
"""

import asyncio

from brain.mechanisms.A2NoradrenergicNTS import A2NoradrenergicNTS
...
class TestOutputKeys:
    ...
    def test_required_keys_present(self):
        mech = A2NoradrenergicNTS()
        result = _run(mech.tick({"prior_results": {}}))
        for key in ['a2_drive', 'pvn_recruitment', 'cea_recruitment', 'bnst_recruitment', 'son_recruitment', 'a1_recruitment', 'visceral_arousal_signal', 'a2_state']:
...
class TestNumericBounds:
    ...
    def test_saturated_inputs_stay_bounded(self):
        mech = A2NoradrenergicNTS()
        prior = {dep: {"_saturated": 1.0} for dep in ['NucleusTractusSolitariusFull', 'BaroreflexBalancer', 'AppetiteNPYBalancer', 'StressActivationAxis', 'AreaPostremaToxinGuard']}
        for _ in range(15):
