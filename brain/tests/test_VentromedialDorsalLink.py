...
class TestOutputKeys:
    def test_required_keys_present(self):
        ...
        for key in ['vmh_defensive_output', 'metabolic_defensive_link', 'estrogen_vmh_modulation', 'glucocorticoid_feedback', 'vmh_dorsal_integrator']:
...
class TestNumericBounds:
    ...
    def test_saturated_inputs_stay_bounded(self):
        mech = VentromedialDorsalLink()
        prior = {dep: {"_saturated": 1.0} for dep in ['AutonomicSecretionLink', 'AmygdalaOutput', 'EstrogenSignal', 'VocalAutonomicLink', 'EnergyConservationMode', 'CRHStressDispatcher']}
        for _ in range(15):
