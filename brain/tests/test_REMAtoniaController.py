"""
Behavioral tests for REMAtoniaController.
"""

import asyncio

from brain.mechanisms.REMAtoniaController import REMAtoniaController
...
class TestOutputKeys:
    def test_required_keys_present(self):
        mech = REMAtoniaController()
        result = _run(mech.tick({"prior_results": {}}))
        for key in ['atonia_level', 'glycine_release', 'rem_active', 'cataplexy_signal', 'motor_inhibition_strength']:
...
class TestNumericBounds:
    ...
    def test_saturated_inputs_stay_bounded(self):
        mech = REMAtoniaController()
        prior = {dep: {"_saturated": 1.0} for dep in []}
        for _ in range(15):
