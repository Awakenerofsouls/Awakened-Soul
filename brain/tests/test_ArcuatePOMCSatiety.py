"""
Behavioral tests for ArcuatePOMCSatiety.

Run:
    pytest brain/tests/test_ArcuatePOMCSatiety.py -v
"""

import asyncio

from brain.mechanisms.ArcuatePOMCSatiety import ArcuatePOMCSatiety
...
class TestOutputKeys:
    ...
    def test_required_keys_present(self):
        mech = ArcuatePOMCSatiety()
        result = _run(mech.tick({"prior_results": {}}))
        for key in ['pomc_drive', 'alpha_msh_release', 'mc4r_engagement', 'beta_endorphin_release', 'satiety_command', 'melanocortin_tone', 'pomc_state']:
...
class TestNumericBounds:
    ...
    def test_saturated_inputs_stay_bounded(self):
        mech = ArcuatePOMCSatiety()
        prior = {dep: {"_saturated": 1.0} for dep in ['ValenceTagger', 'LeptinProxy', 'AppetiteNPYBalancer', 'InsulinProxy']}
        for _ in range(15):
