...
class TestOutputKeys:
    def test_required_keys_present(self):
        ...
        for key in ['heat_defense_signal', 'sympathetic_heat_output', 'arousal_from_homeostasis', 'body_temperature_drive', 'posterior_integrator']:
...
class TestNumericBounds:
    ...
    def test_saturated_inputs_stay_bounded(self):
        mech = PosteriorHomeostaticOutput()
        prior = {dep: {"_saturated": 1.0} for dep in ['CoreTemperatureMonitor', 'HistamineArousalBooster', 'AnteriorHypothalamicCooling', 'AmbientTemperatureRelay', 'OrexinWakePromoter']}
        for _ in range(15):
