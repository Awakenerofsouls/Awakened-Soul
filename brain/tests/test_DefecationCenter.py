...
class TestOutputKeys:
    def test_required_keys_present(self):
        ...
        for key in ['colon_motility', 'rectal_urgency', 'sphincter_coordination', 'parasympathetic_colic_drive', 'emotional_colic_response', 'brain_defecation_urgency']:
...
class TestNumericBounds:
    ...
    def test_saturated_inputs_stay_bounded(self):
        mech = DefecationCenter()
        prior = {dep: {"_saturated": 1.0} for dep in ['AmygdalaOutput', 'MedialPrefrontal', 'GutSignalRelay', 'VagalRestPromoter', 'CRHStressDispatcher']}
        for _ in range(15):
