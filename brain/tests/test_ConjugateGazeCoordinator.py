...
class TestOutputKeys:
    def test_required_keys_present(self):
        ...
        for key in ['gaze_shift_command', 'gaze_target_x', 'gaze_target_y', 'saccade_initiation', 'orienting_priority']:
...
class TestNumericBounds:
    ...
    def test_saturated_inputs_stay_bounded(self):
        mech = ConjugateGazeCoordinator()
        prior = {dep: {"_saturated": 1.0} for dep in ['FrontalEyeFields', 'SNprInhibition', 'AuditoryOrienting', 'ArousalRegulator', 'VisualSalienceMap']}
        for _ in range(15):
