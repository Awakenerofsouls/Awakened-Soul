...
class TestOutputKeys:
    def test_required_keys_present(self):
        ...
        for key in ['heat_production_drive', 'heat_loss_drive', 'sexual_motivation', 'preoptic_sleep_signal', 'thermoregulatory_setpoint']:
...
class TestNumericBounds:
    ...
    def test_saturated_inputs_stay_bounded(self):
        mech = ThermoSexualBalancer()
        prior = {dep: {"_saturated": 1.0} for dep in ['VLPOSleepActive', 'CoreTemperatureMonitor', 'EstrogenSignal', 'TestosteroneSignal', 'AmbientTemperatureRelay', 'CRHStressDispatcher']}
        for _ in range(15):
