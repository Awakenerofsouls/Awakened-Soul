...
class TestOutputKeys:
    def test_required_keys_present(self):
        ...
        for key in ['hpa_hpg_tradeoff', 'reproductive_suppression', 'stress_attenuation', 'defensive_priority', 'survival_reproduction_balance']:
...
class TestNumericBounds:
    ...
    def test_saturated_inputs_stay_bounded(self):
        mech = DefensiveReproductiveLink()
        prior = {dep: {"_saturated": 1.0} for dep in ['GnRHReintegration', 'AutonomicSecretionLink', 'EstrogenSignal', 'TestosteroneSignal', 'CRHStressDispatcher']}
        for _ in range(15):
