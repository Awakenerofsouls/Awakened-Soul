...
class TestOutputKeys:
    def test_required_keys_present(self):
        ...
        for key in ['ltn_integrator', 'motivational_weight', 'drive_coordination', 'ltn_threat_response', 'lateral_tuberal_composite']:
...
class TestNumericBounds:
    ...
    def test_saturated_inputs_stay_bounded(self):
        mech = LateralTuberalNucleusOutput()
        prior = {dep: {"_saturated": 1.0} for dep in ['FacialGradientSensor', 'AmygdalaOutput', 'ArousalRegulator', 'FeedingStressIntegrator', 'CRHStressDispatcher', 'ThermoSexualBalancer']}
        for _ in range(15):
