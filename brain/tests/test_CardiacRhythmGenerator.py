"""
Behavioral tests for CardiacRhythmGenerator.

Run:
    pytest brain/tests/test_CardiacRhythmGenerator.py -v
"""

import asyncio

from brain.mechanisms.CardiacRhythmGenerator import CardiacRhythmGenerator
...
class TestOutputKeys:
    ...
    def test_required_keys_present(self):
        mech = CardiacRhythmGenerator()
        result = _run(mech.tick({"prior_results": {}}))
        for key in ['heart_rate_proxy', 'hr_variability', 'vagal_tone', 'cardiac_baroreflex_gain', 'cardiac_dysregulation']:
...
class TestNumericBounds:
    ...
    def test_saturated_inputs_stay_bounded(self):
        mech = CardiacRhythmGenerator()
        prior = {dep: {"_saturated": 1.0} for dep in ['RespiratoryPacemaker', 'ArousalRegulator', 'VitalCoreRegulator']}
        for _ in range(15):
