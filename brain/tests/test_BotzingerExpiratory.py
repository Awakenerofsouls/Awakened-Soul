"""
Behavioral tests for BotzingerExpiratory.

Run:
    pytest brain/tests/test_BotzingerExpiratory.py -v
"""

import asyncio

from brain.mechanisms.BotzingerExpiratory import BotzingerExpiratory
...
class TestOutputKeys:
    ...
    def test_required_keys_present(self):
        mech = BotzingerExpiratory()
        result = _run(mech.tick({"prior_results": {}}))
        for key in ['botc_drive', 'post_i_drive', 'aug_e_drive', 'expiratory_inhibition', 'active_expiration_engaged', 'phase_marker', 'botc_state']:
...
class TestNumericBounds:
    ...
    def test_saturated_inputs_stay_bounded(self):
        mech = BotzingerExpiratory()
        prior = {dep: {"_saturated": 1.0} for dep in ['PreBotzingerInspiration', 'VitalCoreRegulator', 'ValenceTagger', 'CarotidBodyChemosensor', 'ArousalRegulator']}
        for _ in range(15):
