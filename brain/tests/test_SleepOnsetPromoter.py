...
class TestOutputKeys:
    def test_required_keys_present(self):
        ...
        for key in ['sleep_onset_probability', 'hippocampal_consolidation', 'cortical_activation_during_sleep', 'cholinergic_rem_drive', 'basal_forebrain_integrator']:
...
class TestNumericBounds:
    ...
    def test_saturated_inputs_stay_bounded(self):
        mech = SleepOnsetPromoter()
        prior = {dep: {"_saturated": 1.0} for dep in ['CircadianDrive', 'HippocampalReplayIntegrator', 'SleepWakeFlipFlop', 'Homeostat', 'REMAtoniaController', 'PassiveQuiescenceMode']}
        for _ in range(15):
