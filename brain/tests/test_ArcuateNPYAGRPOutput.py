"""
Behavioral tests for ArcuateNPYAGRPOutput.

Run:
    pytest brain/tests/test_ArcuateNPYAGRPOutput.py -v
"""

import asyncio

from brain.mechanisms.ArcuateNPYAGRPOutput import ArcuateNPYAGRPOutput
...
class TestOutputKeys:
    ...
    def test_required_keys_present(self):
        mech = ArcuateNPYAGRPOutput()
        result = _run(mech.tick({"prior_results": {}}))
        for key in ['npy_level', 'agrp_output', 'hunger_drive', 'mc4r_competition', 'arcuate_hunger_integrator']:
...
class TestNumericBounds:
    ...
    def test_saturated_inputs_stay_bounded(self):
        mech = ArcuateNPYAGRPOutput()
        prior = {dep: {"_saturated": 1.0} for dep in ['ArcuatePOMCOutput', 'GutSignalRelay', 'InsulinSignal', 'EnergyConservationMode', 'GlucoseMonitor']}
        for _ in range(15):
