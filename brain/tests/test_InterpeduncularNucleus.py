"""
Behavioral tests for InterpeduncularNucleus.
"""

import asyncio

from brain.mechanisms.InterpeduncularNucleus import InterpeduncularNucleus
...
class TestOutputKeys:
    def test_required_keys_present(self):
        mech = InterpeduncularNucleus()
        result = _run(mech.tick({"prior_results": {}}))
        for key in ['ipn_drive', 'raphe_5ht_modulation', 'dorsal_tegmental_signal', 'aversion_amplification', 'ipn_state']:
...
class TestNumericBounds:
    ...
    def test_saturated_inputs_stay_bounded(self):
        mech = InterpeduncularNucleus()
        prior = {dep: {"_saturated": 1.0} for dep in ['MedialHabenula']}
        for _ in range(15):
