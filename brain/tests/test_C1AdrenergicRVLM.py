"""
Behavioral tests for C1AdrenergicRVLM.

Run:
    pytest brain/tests/test_C1AdrenergicRVLM.py -v
"""

import asyncio

from brain.mechanisms.C1AdrenergicRVLM import C1AdrenergicRVLM
...
class TestOutputKeys:
    ...
    def test_required_keys_present(self):
        mech = C1AdrenergicRVLM()
        result = _run(mech.tick({"prior_results": {}}))
        for key in ['c1_drive', 'spinal_iml_sympathetic', 'adrenal_epinephrine_drive', 'glucose_counterreg_active', 'pvn_acth_recruitment', 'bp_elevation_drive', 'c1_state']:
...
class TestNumericBounds:
    ...
    def test_saturated_inputs_stay_bounded(self):
        mech = C1AdrenergicRVLM()
        prior = {dep: {"_saturated": 1.0} for dep in ['BaroreflexBalancer', 'AppetiteNPYBalancer', 'HemorrhageProxy', 'VitalCoreRegulator', 'StressActivationAxis', 'ValenceTagger', 'CarotidBodyChemosensor', 'HypoglycemiaProxy']}
        for _ in range(15):
