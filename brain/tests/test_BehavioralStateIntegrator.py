"""
Behavioral tests for BehavioralStateIntegrator.

Run:
    pytest brain/tests/test_BehavioralStateIntegrator.py -v
"""

import asyncio

from brain.mechanisms.BehavioralStateIntegrator import BehavioralStateIntegrator
...
class TestOutputKeys:
    ...
    def test_required_keys_present(self):
        mech = BehavioralStateIntegrator()
        result = _run(mech.tick({"prior_results": {}}))
        for key in ['cortical_arousal_level', 'eeg_state', 'behavioral_state', 'consciousness_index', 'attention_modulation']:
...
class TestNumericBounds:
    ...
    def test_saturated_inputs_stay_bounded(self):
        mech = BehavioralStateIntegrator()
        prior = {dep: {"_saturated": 1.0} for dep in ['CholinergicREMOn', 'DorsalRapheSerotonin', 'PassiveQuiescenceMode', 'REMAtoniaController', 'ArousalRegulator', 'HistamineArousalBooster']}
        for _ in range(15):
