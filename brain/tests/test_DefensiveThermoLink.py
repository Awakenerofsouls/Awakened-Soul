...
class TestOutputKeys:
    def test_required_keys_present(self):
        ...
        for key in ['defensive_cooling_signal', 'thermal_pain_suppression', 'autonomic_cooling_effort', 'fever_threshold_proximity', 'sleep_permissiveness']:
...
class TestNumericBounds:
    ...
    def test_saturated_inputs_stay_bounded(self):
        mech = DefensiveThermoLink()
        prior = {dep: {"_saturated": 1.0} for dep in ['CRHStressDispatcher', 'PeripheralTemperature', 'CoreTemperatureMonitor', 'AmbientTemperatureRelay']}
        for _ in range(15):
