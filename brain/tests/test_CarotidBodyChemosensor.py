"""
Behavioral tests for CarotidBodyChemosensor.

Run:
    pytest brain/tests/test_CarotidBodyChemosensor.py -v
"""

import asyncio

from brain.mechanisms.CarotidBodyChemosensor import CarotidBodyChemosensor
...
class TestOutputKeys:
    ...
    def test_required_keys_present(self):
        mech = CarotidBodyChemosensor()
        result = _run(mech.tick({"prior_results": {}}))
        for key in ['po2_proxy', 'pco2_proxy', 'ph_proxy', 'chemoreceptor_drive', 'atp_release', 'hypoxia_response_active', 'chronic_intermittent_sensitization', 'hvr_gain', 'catecholamines', 'nts_afferent_firing', 'intermittent_pattern']:
...
class TestNumericBounds:
    ...
    def test_saturated_inputs_stay_bounded(self):
        mech = CarotidBodyChemosensor()
        prior = {dep: {"_saturated": 1.0} for dep in ['RespiratoryPacemaker', 'ArousalRegulator', 'VitalCoreRegulator', 'StressActivationAxis']}
        for _ in range(15):
