"""
Behavioral tests for AnteriorThalamicPapez.

Run:
    pytest brain/tests/test_AnteriorThalamicPapez.py -v
"""

import asyncio

from brain.mechanisms.AnteriorThalamicPapez import AnteriorThalamicPapez
...
class TestOutputKeys:
    ...
    def test_required_keys_present(self):
        mech = AnteriorThalamicPapez()
        result = _run(mech.tick({"prior_results": {}}))
        for key in ['ad_drive', 'av_drive', 'am_drive', 'head_direction_relay', 'retrosplenial_relay', 'cingulate_relay', 'papez_theta_coherence', 'atn_state']:
...
class TestNumericBounds:
    ...
    def test_saturated_inputs_stay_bounded(self):
        mech = AnteriorThalamicPapez()
        prior = {dep: {"_saturated": 1.0} for dep in ['SubiculumOutput', 'LocomotionProxy', 'MammillaryBodyMemory', 'MedialSeptumTheta', 'ThalamicReticularNucleus']}
        for _ in range(15):
