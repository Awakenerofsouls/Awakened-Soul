"""
Behavioral tests for ArousalRegulator.

Run:
    pytest brain/tests/test_ArousalRegulator.py -v
"""

import asyncio

from brain.mechanisms.ArousalRegulator import ArousalRegulator
...
class TestOutputKeys:
    ...
    def test_required_keys_present(self):
        mech = ArousalRegulator()
        result = _run(mech.tick({"prior_results": {}}))
        for key in ['arousal_level', 'creative_mode', 'reflective_mode', 'hyperaroused', 'hypoaroused', 'tonic_level', 'phasic_burst_active', 'mode']:
...
class TestNumericBounds:
    ...
    def test_saturated_inputs_stay_bounded(self):
        mech = ArousalRegulator()
        prior = {dep: {"_saturated": 1.0} for dep in ['PredictionErrorDrift', 'Homeostat']}
        for _ in range(15):
