...
class TestOutputKeys:
    def test_required_keys_present(self):
        ...
        for key in ['sensory_gate_output', 'thalamic_relay_fidelity', 'novel_stimulus_flag', 'pain_inhibition_input', 'reticular_alert_level']:
...
class TestNumericBounds:
    ...
    def test_saturated_inputs_stay_bounded(self):
        mech = ReticularSensoryPreFilter()
        prior = {dep: {"_saturated": 1.0} for dep in ['VLPOSleepActive', 'AuditoryOrienting', 'DescendingPainGate', 'ArousalRegulator', 'VisualSalienceMap']}
        for _ in range(15):
