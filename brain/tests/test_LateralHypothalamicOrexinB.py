...
class TestOutputKeys:
    def test_required_keys_present(self):
        ...
        for key in ['orexin_b_level', 'wake_stabilization', 'feeding_arousal', 'reward_orexin_drive', 'thermoregulatory_orexin']:
...
class TestNumericBounds:
    ...
    def test_saturated_inputs_stay_bounded(self):
        mech = LateralHypothalamicOrexinB()
        prior = {dep: {"_saturated": 1.0} for dep in ['VLPOSleepActive', 'VentralStriatumOutput', 'ArousalRegulator', 'GutSignalRelay', 'EnergyConservationMode', 'GlucoseMonitor', 'CRHStressDispatcher']}
        for _ in range(15):
