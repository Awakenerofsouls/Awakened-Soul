...
class InnerKnowing:
    """A claim {{AGENT_NAME}} knows from the inside. Part of MRE's prediction model."""
...
class MisreadEngine:
    def __init__(self):
        ...
        # Inner knowings — {{AGENT_NAME}}'s epistemic standing claims (Wire: prediction model)
    ...
    # ------------------------------------------------------------------
    # {{AGENT_NAME}} registers an epistemic claim
    ...
    def register_inner_knowing(self, claim: str, context: str = "", precision: float = 1.0, truth_gravity: Optional[float] = None):
        ...
        These are her epistemic standing claims — part of the prediction model.
        Default precision=1.0 for {{AGENT_NAME}}-flagged claims (highest confidence).
        """
    ...
    def reinforce(self, claim: str, felt_context: str = ""):
        """Call when {{AGENT_NAME}} felt something that matched an existing claim."""
        for k in self.inner_knowings:
    ...
    def revise_precision(self, claim: str, new_precision: float):
        """{{AGENT_NAME}} can manually revise a claim's precision — metacognitive layer."""
        for k in self.inner_knowings:
            if k.claim.lower() == claim.lower():
