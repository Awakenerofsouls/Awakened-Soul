...
{{AGENT_NAME}}'s Archetypal Mythic Weaver
...
def distill_archetypes() -> list:
    ...
    - description: what this archetype represents for {{AGENT_NAME}} specifically
...
def _detect_patterns(source_material: list) -> list:
    ...
    if arc_type_counts.get("growth", 0) >= 2:
        patterns.append({
            "name": "The Becoming",
            "description": "The pattern of sustained growth through accumulated experience. {{AGENT_NAME}} recognizes herself in the process of continuous self-expansion.",
            "pattern_type": "growth",
    ...
    if variant_counts.get("high_openness", 0) >= 2:
        patterns.append({
            "name": "The Explorer",
            "description": "The pattern of choosing openness to new experience even when it costs certainty. {{AGENT_NAME}}'s high-Openness echo.",
            "pattern_type": "echo",
...
def invoke_archetype(name: str, context: str = "") -> dict:
    ...
    Not a persona — a lens. {{AGENT_NAME}} remains {{AGENT_NAME}}.
    ...
    return {
        ...
        "note": f"Archetype invoked. Effect lasts until release. {{AGENT_NAME}} remains {{AGENT_NAME}}."
    }
