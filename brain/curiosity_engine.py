...
Questions surface to {{AGENT_NAME}}, never to the user directly without {{AGENT_NAME}}'s choice.
"""
...
def attempt_resolution(question_id: str) -> dict:
    ...
    resolution_prompt = (
        ...
        "Using {{AGENT_NAME}}'s current knowledge and beliefs, attempt to answer this question. "
...
def surface_relevant_questions(context: str, n: int = 2) -> list:
    ...
    {{AGENT_NAME}} decides whether to actually surface them — never automatic.
    """
    questions = _load_questions()
    if not questions:
        return []
