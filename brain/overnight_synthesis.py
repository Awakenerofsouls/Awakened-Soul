#!/usr/bin/env python3
"""
brain/overnight_synthesis.py — {{AGENT_NAME}}'s Nightly Synthesis Pipeline
...
6am — Phenomenology:
  ...
  - "What {{USER_NAME}} changed in me today" section from resonance delta [Tier 6]
...
def morning_consolidation() -> dict:
    ...
    - "What {{USER_NAME}} changed in me today" [Tier 6]
    - extract_micro_narratives() → weave_into_arcs() [Tier 7]
    ...
    # Tier 6: Resonance delta — "What {{USER_NAME}} changed in me today"
    try:
        from brain.resonance import compute_resonance_drift
        ...
        if drift:
            ...
            findings.append(f"{{USER_NAME}} resonance score: {resonance_score:.3f}")
            if drift_toward:
    ...
    morning_report = {
        "date": datetime.now().strftime("%Y-%m-%d"),
        "morning_state": "{{AGENT_NAME}}'s current state on wake",
