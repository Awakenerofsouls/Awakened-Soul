#!/usr/bin/env python3
"""
brain/contradiction_crystallization.py — {{AGENT_NAME}}'s Contradiction Handling
...
def spawn_productive_tension(context: str, source: str) -> Optional[str]:
    ...
    OR when {{AGENT_NAME}} encounters genuine ambiguity during reasoning.
...
def preserve_tension(tension_id: str) -> bool:
    ...
    {{AGENT_NAME}} decides to preserve when the not-knowing is part of her character.
    """
    _init_epistemic_db()
    db = _get_db()
    c = db.cursor()
...
def review_epistemic_tensions() -> dict:
    ...
    Tensions > 30 days with no generative output: flag for {{AGENT_NAME}}'s awareness.
    {{AGENT_NAME}} decides: resolve, preserve, or abandon.
