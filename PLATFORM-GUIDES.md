...
Complete integration guides for running {{AGENT_NAME}} across every major AI platform.
...
// ~/.openclaw/config.json
{
  "agent": {
    "name": "{{AGENT_NAME}}",
    "model": "llm_provider-portal/LLMProvider-M2.5",
    "temperature": 0.7
  },
...
openclaw config set voice.voice agent
```
...
memory = AgentMemory(
    ...
    embedder="llm_provider"
)

# Store
memory.remember(user_id, "{{USER_NAME}} likes dark chocolate")

# Recall
context = memory.recall("what does {{USER_NAME}} like")
```
...
3. Select "{{AGENT_NAME}} Memory" MCP server
...
response = client.audio.speech.create(
    model="tts-1-hd",
    voice="agent",  # Custom voice if available
    input="Hello, I'm {{AGENT_NAME}}."
)
...
- **Platform:** OpenClaw
- **Model:** LLMProvider M2.5 or Claude Sonnet
