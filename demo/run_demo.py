#!/usr/bin/env python3
"""
run_demo.py — Nexus {{AGENT_NAME}} Framework Demo
...
# ── 1. Identity load ─────────────────────────────────────────────────────────
print("=" * 60)
print("NEXUS AGENT FRAMEWORK — Demo Run")
print("=" * 60)
...
# Add sample entities: {{USER_NAME}} builds {{AGENT_NAME}}
user_id = add_node(node_id="user", label="{{USER_NAME}}", node_type="person",
                     properties={"role": "operator"})
agent_id = add_node(node_id="agent", label="{{AGENT_NAME}}", node_type="agent",
                   properties={"type": "framework"})
rel_id = add_edge(from_id=user_id, to_id=agent_id, edge_type="builds")
