#!/usr/bin/env python3
"""
AGENT MULTIUSER — Multi-User Support
...
MULTIUSER_DB = AGENT_DIR / "multiuser.db"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="{{AGENT_NAME}} Multi-User")
    ...
    if args.command == 'list':
        ...
        if len(args.args) < 2:
            print("Usage: multiuser.py add <user_id> <name> [pronouns]")
            return
        ...
        if not args.args:
            print("Usage: multiuser.py profile <user_id>")
            return
        ...
        if not args.args:
            print("Usage: multiuser.py know <user_id>")
            return
        
        user_id = args.args[0]
        info = manager.what_agent_knows(user_id)
        
        print(f"\n🧠 WHAT AGENT KNOWS ABOUT {info.get('user', 'user').upper()}")
        print("=" * 40)
        ...
        if not args.args:
            print("Usage: multiuser.py delete <user_id>")
            return
